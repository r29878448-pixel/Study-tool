...
import { 
  ...
  RotateCcw, ImageIcon, Copy, Shield
...
const TempAccessHandler = () => {
    ...
    const courseName = courses.find(c => c.id === id)?.title || "Batch Content";
        ...
        <div className="min-h-screen flex flex-col items-center justify-center p-8 bg-gray-50">
            <div className="bg-white p-10 rounded-[40px] shadow-2xl border border-gray-100 max-w-sm w-full animate-slide-up text-center">
                <div className="w-20 h-20 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-6">
                    {/* Fix: Added missing Shield icon import from ./components/Icons */}
                ...
                <p className="text-gray-500 text-sm font-medium mb-8 leading-relaxed">{status === 'verifying' ? 'Validating your temporary access token...' : status === 'success' ? 'Redirecting you to the batch content.' : `Confirming 24h access for: ${courseName}`}</p>
...
const ContentManager = ({ course, onClose }: { course: Course, onClose: () => void }) => {
    ...
    const addSubject = () => {
        const title = prompt('Subject Name (e.g. Maths):');
        const iconText = prompt('Icon Letter (e.g. M):') || 'S';
    ...
    const addVideo = (sId: string, cId: string) => {
        ...
        const url = prompt('Video URL or Note Link:');
        const dur = prompt('Duration (e.g. 45:00):') || '10:00';
        ...
        if (title && url) setSubjects(subjects.map(s => s.id === sId ? { ...s, chapters: s.chapters.map(c => c.id === cId ? { ...c, videos: [...c.videos, { id: Date.now().toString(), title, filename: url, duration: dur, type, date: 'New' }] } : c) } : s));
    };
            ...
            <div className="bg-white w-full max-w-2xl rounded-[32px] p-8 max-h-[85vh] flex flex-col shadow-2xl animate-slide-up overflow-hidden">
                <div className="flex justify-between items-center mb-6 border-b pb-4">
                    <h2 className="text-xl font-bold flex items-center gap-2 text-gray-800">
                      ...
                      {activeChapter ? activeChapter.title : activeSubject ? activeSubject.title : 'Edit Subjects'}
                    </h2>
                ...
                <div className="flex-1 overflow-y-auto space-y-4 no-scrollbar">
                    {activeChapterId && activeSubjectId ? (
                        <div className="space-y-3">
                            <button onClick={() => addVideo(activeSubjectId, activeChapterId)} className="w-full py-4 border-2 border-dashed border-blue-100 rounded-2xl text-blue-600 font-bold hover:bg-blue-50 transition-all">+ Add Chapter Node</button>
                ...
                <div className="mt-6 pt-6 border-t flex gap-3">
                    ...
                    <button onClick={handleSave} className="flex-1 py-4 bg-[#0056d2] text-white font-bold rounded-2xl shadow-lg transition-all">Save Batch</button>
                </div>
            </div>
        </div>
    );
...
const AdminPanel = () => {
    ...
    const handleSaveSettings = (e: React.FormEvent) => { e.preventDefault(); updateSettings(settingsForm); alert("System name updated."); };
    
    const copyBatchData = () => {
        const json = JSON.stringify(courses, null, 2);
        navigator.clipboard.writeText(json);
        alert("Batch Data copied! Send this to the developer to make your data PERMANENT for everyone.");
    };
        ...
        <div className="pb-24 pt-24 px-4 min-h-screen bg-[#f8fafc]">
             <div className="max-w-6xl mx-auto">
                 <div className="flex bg-white p-1.5 rounded-lg shadow-sm border border-gray-200 mb-8 max-w-md">
                    {(['batches', 'settings'] as const).map(t => (<button key={t} onClick={() => setTab(t)} className={`flex-1 py-2 font-bold capitalize transition-all rounded-md text-xs ${tab === t ? 'bg-[#0056d2] text-white shadow-sm' : 'text-gray-400 hover:bg-gray-50'}`}>{t}</button>))}
                 </div>

                 {tab === 'batches' && (
                     <div className="space-y-4">
                         <button onClick={() => { setEditing(null); setShowModal(true); }} className="w-full py-10 bg-white border-2 border-dashed border-blue-200 rounded-xl text-[#0056d2] font-bold flex flex-col items-center justify-center gap-2 hover:bg-blue-50 transition-all"><Plus className="w-8 h-8" /><span>NEW BATCH NODE</span></button>
                 ...
                 {tab === 'settings' && (
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
                        <h2 className="text-xl font-bold text-gray-800 mb-6">Master Control</h2>
                        <form onSubmit={handleSaveSettings} className="space-y-6">
                            <div className="space-y-2"><label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Portal Branding</label><input value={settingsForm.appName} onChange={e => setSettingsForm({...settingsForm, appName: e.target.value})} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-md font-bold text-sm" /></div>
                            <button type="submit" className="w-full py-3 bg-[#0056d2] text-white font-black rounded-md shadow-md text-sm uppercase">Update Portal Name</button>
                        </form>
                        <div className="mt-12 pt-8 border-t border-gray-100 space-y-4">
                            <h3 className="text-blue-600 font-bold">Data Migration Tool</h3>
                            <button onClick={copyBatchData} className="flex items-center gap-2 px-6 py-3 bg-blue-50 text-blue-600 border border-blue-200 rounded-xl font-bold text-sm hover:bg-blue-600 hover:text-white transition-all w-full justify-center shadow-sm">
                                <Copy className="w-4 h-4" /> Copy All Batch Data (to send to dev)
                            </button>
                            <h3 className="text-red-600 font-bold">Hard Reset</h3>
                            <button onClick={clearAllData} className="flex items-center gap-2 px-6 py-3 bg-red-50 text-red-600 border border-red-200 rounded-xl font-bold text-sm hover:bg-red-600 hover:text-white transition-all w-full justify-center">
                                <RotateCcw className="w-4 h-4" /> NUCLEAR RESET (Clear Local Stale Data)
                            </button>
                        </div>
                    </div>
                 )}
                 ...
                 <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div className="bg-white w-full max-w-lg rounded-lg p-6 shadow-2xl max-h-[90vh] overflow-y-auto border border-gray-200">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-xl font-bold text-gray-800">{editing ? 'Edit Configuration' : 'Add New Node'}</h2>
                        ...
                        <form onSubmit={handleSaveCourse} className="space-y-4">
                            ...
                            <input value={form.image} onChange={e => setForm({ ...form, image: e.target.value })} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-md text-xs" placeholder="Image URL (Banner)" />
                            <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-md text-xs min-h-[80px]" placeholder="Brief Summary" required />
                            <div className="grid grid-cols-2 gap-4">
                                <input value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} className="p-3 bg-gray-50 border border-gray-200 rounded-md text-xs uppercase font-bold" placeholder="Batch Level" required />
                                <div className="flex items-center justify-between px-3 py-3 bg-gray-50 rounded-md border border-gray-200 cursor-pointer" onClick={() => setForm({...form, isPaid: !form.isPaid})}>
                                    <span className="text-xs font-bold text-gray-500">Paid Node</span>
                            ...
                            <button type="submit" className="w-full py-3 bg-[#0056d2] text-white font-bold rounded-md shadow-md text-sm uppercase">Commit Batch</button>
                        </form>
                    </div>
                 </div>
             )}
            ...
            <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {courses.length === 0 ? (
                    <div className="col-span-full flex flex-col items-center justify-center py-32 text-gray-400">
                        ...
                        <p className="uppercase font-black tracking-widest text-[10px]">No active batches. Please check back later.</p>
                    </div>
                ) : courses.map(c => (
                    <div key={c.id} className="bg-white rounded-[40px] overflow-hidden border border-gray-100 shadow-sm flex flex-col hover:shadow-lg transition-all">
                        ...
                        <div className="p-7 flex-1 flex flex-col justify-end"><Link to={`/course/${c.id}`} className="w-full py-4 bg-[#0056d2] text-white text-center text-sm font-black rounded-2xl shadow-xl shadow-blue-200 uppercase tracking-widest">Open Batch</Link></div>
                    </div>
                ))}
            </div>
        </div>
                ...
                {subject.chapters.map((chap, idx) => (
                    <div key={chap.id} className="bg-white border border-gray-100 rounded-[40px] p-8 shadow-sm space-y-4">
                        <div className="flex justify-between items-center">
                            <div>
                                <span className="text-[#0056d2] text-[10px] font-black uppercase tracking-widest">Module {idx + 1}</span>
    ...
    return (
        <div className="min-h-screen bg-black text-white flex flex-col">
            <div className="p-4 flex items-center gap-4 bg-gray-900 border-b border-white/10">
                ...
                <h1 className="text-lg font-bold truncate">{activeVideo?.title || "Stream"}</h1>
            </div>
            <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
                <div className="flex-1 bg-black flex items-center justify-center p-4">
                    {activeVideo ? <VideoPlayer src={activeVideo.filename} title={activeVideo.title} onBack={() => navigate(-1)} className="w-full max-w-5xl shadow-2xl" /> : <div className="text-gray-500">Wait...</div>}
                </div>
                <div className="w-full lg:w-96 bg-gray-900 overflow-y-auto border-l border-white/10 p-6 no-scrollbar">
                    <h3 className="font-bold mb-6 text-gray-400 uppercase text-xs tracking-widest">Batch Playlist</h3>
    ...
    return (
        <div className="pb-24 pt-24 px-6 min-h-screen bg-[#f8fafc]">
            <div className="max-w-2xl mx-auto space-y-8">
                ...
                {currentUser.role === UserRole.ADMIN && <Link to="/admin" className="block w-full py-8 bg-gradient-to-r from-[#0056d2] to-[#003ea1] text-white rounded-[50px] text-center font-black shadow-2xl uppercase tracking-[0.3em] text-xl active:scale-95 transition-transform">Control Center</Link>}
            </div>
        </div>
    );
};
...
const Login = () => {
    ...
    return (
        <div className="min-h-screen flex flex-col items-center justify-center p-8 bg-[#0056d2]">
            ...
            <div className="bg-white p-12 rounded-[70px] w-full max-w-md shadow-2xl animate-fade-in">
                <h2 className="text-2xl font-black text-gray-800 mb-12 text-center uppercase">Identity</h2>
                <form onSubmit={e => { e.preventDefault(); if(isS) signup(f.name, f.email, '', f.pass); else if (!login(f.email, f.pass)) alert('Login failed. Check inputs.'); }} className="space-y-5">
                    ...
                    <button className="w-full py-6 bg-[#0056d2] text-white font-black rounded-[35px] shadow-2xl shadow-blue-200 uppercase tracking-widest mt-8 active:scale-95 transition-all">Continue</button>
                </form>
                <button onClick={() => setIsS(!isS)} className="w-full mt-12 text-[10px] text-[#0056d2] font-black uppercase text-center">{isS ? '< Login' : 'Register Identity >'}</button>
            </div>
        </div>
    );
};
