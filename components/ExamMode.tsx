...
import { Timer, CheckCircle, AlertCircle, ArrowLeft, Loader2, List, PlayCircle, Bot, Save, LogOut, RotateCcw, Brain, Sparkles } from './Icons';
...
const ExamMode = () => {
  ...
  const timeLeftRef = useRef(600); // Ref to track time without triggering re-renders in effects
  ...
  // Fixed: Removed timeLeft from dependency array to prevent race condition (timer ticking kills debounce)
  // Uses timeLeftRef.current to get the latest time
  useEffect(() => {
  ...
  }, [answers, currentQuestionIdx, view, loading, isFinished]);
  ...
  if (resumePrompt) {
    return (
      <div className="min-h-screen bg-[#050508] flex items-center justify-center p-6">
         <div className="glass p-10 rounded-[50px] border border-indigo-500/20 bg-gray-900/50 backdrop-blur-xl max-w-sm w-full text-center shadow-[0_0_30px_rgba(79,70,229,0.15)]">
            ...
            <div className="flex gap-4">
               ...
               <button onClick={handleResume} className="flex-1 py-4 bg-indigo-600 text-white font-bold rounded-2xl hover:scale-105 transition-all shadow-[0_0_20px_rgba(79,70,229,0.3)]">
                  Resume
               </button>
            </div>
         </div>
  ...
  if (view === 'selection') {
    return (
       <div className="min-h-screen bg-[#050508] p-6 pt-24 overflow-hidden relative">
          <div className="absolute inset-0 opacity-10 pointer-events-none" style={{ backgroundImage: 'linear-gradient(#4f46e5 1px, transparent 1px), linear-gradient(90deg, #4f46e5 1px, transparent 1px)', backgroundSize: '40px 40px' }}></div>
          <div className="max-w-2xl mx-auto relative z-10">
              <button onClick={() => navigate(`/course/${course.id}`)} className="p-3 bg-white/5 backdrop-blur-lg rounded-2xl text-white mb-10 transition-transform hover:scale-110 border border-white/10">
                <ArrowLeft />
              </button>
              ...
              <div className="grid gap-6">
                <button onClick={startAiExam} className="bg-white/5 backdrop-blur-md p-8 rounded-[40px] border border-indigo-500/30 text-left group hover:border-indigo-500 transition-all hover:bg-indigo-500/5 hover:shadow-[0_0_30px_rgba(79,70,229,0.1)]">
                    <div className="flex justify-between items-center mb-4">
                        <div className="p-3 rounded-2xl bg-indigo-500 text-white transition-transform group-hover:scale-110 shadow-lg">
                ...
                {course.exams?.map((exam) => (
                  <button key={exam.id} onClick={() => startManualExam(exam)} className="bg-white/5 backdrop-blur-sm p-6 rounded-[35px] border border-white/5 text-left flex items-center justify-between hover:bg-white/10 transition-all group">
                      <div>
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#050508] flex flex-col items-center justify-center p-8 text-center relative overflow-hidden">
        <div className="absolute inset-0 opacity-10 pointer-events-none" style={{ backgroundImage: 'linear-gradient(#4f46e5 1px, transparent 1px), linear-gradient(90deg, #4f46e5 1px, transparent 1px)', backgroundSize: '40px 40px' }}></div>
  ...
  if (isFinished) {
    ...
    return (
      <div className="min-h-screen bg-[#050508] flex items-center justify-center p-8 relative overflow-hidden">
        <div className="absolute inset-0 opacity-10 pointer-events-none" style={{ backgroundImage: 'linear-gradient(#4f46e5 1px, transparent 1px), linear-gradient(90deg, #4f46e5 1px, transparent 1px)', backgroundSize: '40px 40px' }}></div>
        <div className="max-w-md w-full text-center bg-white/5 backdrop-blur-xl p-12 rounded-[60px] border border-indigo-500/20 animate-slide-up relative z-10 shadow-[0_0_50px_rgba(79,70,229,0.1)]">
          <div className="w-24 h-24 bg-emerald-500 shadow-[0_0_30px_rgba(16,185,129,0.4)] rounded-[30px] flex items-center justify-center mx-auto mb-8">
          ...
          <button onClick={() => navigate(`/course/${course.id}`)} className="w-full py-5 bg-indigo-600 text-white font-bold rounded-2xl shadow-lg tracking-widest uppercase hover:scale-[1.02] active:scale-95 transition-all">
            RETURN TO GRID
          </button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#050508] text-white flex flex-col relative">
        ...
        <div className="p-6 bg-[#050508]/80 backdrop-blur-md border-b border-white/5 flex justify-between items-center sticky top-0 z-20">
            <div className="flex items-center gap-4">
                <div className="text-indigo-400 font-mono text-xl font-bold px-4 py-2 bg-white/5 rounded-xl border border-indigo-500/20 shadow-[0_0_15px_rgba(79,70,229,0.2)]">
                  {formatTime(timeLeft)}
                </div>
            </div>
        ...
        <div className="flex-1 p-8 max-w-3xl mx-auto w-full animate-fade-in flex flex-col relative z-10" key={currentQuestionIdx}>
            ...
            <div className="grid gap-4">
                {q.options.map((opt, idx) => (
                    <button 
                      ...
                      className={`p-6 rounded-[30px] border text-left transition-all font-medium text-lg flex items-center gap-5 group ${answers[currentQuestionIdx] === idx ? 'bg-indigo-600 border-indigo-400 shadow-[0_0_20px_rgba(79,70,229,0.4)] scale-[1.02]' : 'bg-white/5 border-white/5 hover:bg-white/10'}`}
                    >
        ...
        <div className="p-6 bg-[#050508]/80 backdrop-blur-md border-t border-white/5 flex justify-between gap-4 sticky bottom-0 z-20">
            <button 
              onClick={() => setCurrentQuestionIdx(prev => Math.max(0, prev - 1))} 
              disabled={currentQuestionIdx === 0}
              className="px-8 py-4 bg-white/5 border border-white/10 rounded-2xl font-bold text-gray-400 hover:text-white disabled:opacity-20 transition-all uppercase tracking-widest text-[10px]"
            >
              Previous
            </button>
            ...
            {currentQuestionIdx === questions.length - 1 ? (
                <button onClick={finishExam} className="px-12 py-4 bg-indigo-600 rounded-2xl font-bold shadow-lg uppercase tracking-widest text-[10px] transition-all hover:scale-105 active:scale-95">
                  Finalize
                </button>
            ) : (
                <button onClick={() => setCurrentQuestionIdx(prev => Math.min(questions.length - 1, prev + 1))} className="px-12 py-4 bg-indigo-600 rounded-2xl font-bold shadow-lg uppercase tracking-widest text-[10px] transition-all hover:scale-105 active:scale-95">
                  Next Node
                </button>
            )}
        </div>
