        ...
        <div className="fixed inset-0 z-[200] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-2xl rounded-[32px] p-8 max-h-[85vh] flex flex-col shadow-2xl animate-slide-up overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto space-y-4 no-scrollbar">
                    {activeChapterId && activeSubjectId ? (
                        <div className="space-y-3">
                            <button onClick={() => addVideo(activeSubjectId, activeChapterId)} className="w-full py-4 border-2 border-dashed border-blue-100 rounded-2xl text-blue-600 font-bold hover:bg-blue-50">+ Add Content</button>
                            {activeChapter?.videos.map(vid => (
                                <div key={vid.id} className="p-4 bg-gray-50 border border-gray-100 rounded-2xl flex justify-between items-center">
                                    <div><p className="text-sm font-bold text-gray-800">{vid.title}</p></div>
                        ...
                        <div className="space-y-3">
                            <button onClick={() => addChapter(activeSubjectId)} className="w-full py-4 border-2 border-dashed border-blue-100 rounded-2xl text-blue-600 font-bold hover:bg-blue-50">+ Add Chapter</button>
                        ...
                        <div className="space-y-3">
                            <button onClick={addSubject} className="w-full py-4 border-2 border-dashed border-blue-100 rounded-2xl text-blue-600 font-bold hover:bg-blue-50">+ Add Subject</button>
                            {subjects.map(sub => (
                ...
                <div className="mt-6 pt-6 border-t flex gap-3">
                    <button onClick={onClose} className="flex-1 py-4 text-gray-500 font-bold hover:bg-gray-50 rounded-2xl">Discard</button>
                    <button onClick={handleSave} className="flex-1 py-4 bg-[#0056d2] text-white font-bold rounded-2xl shadow-lg">Commit changes</button>
                </div>
            </div>
        </div>
    );
...
const AdminPanel = () => {
    ...
    useEffect(() => {
        if (editing) {
          ...
          setForm({ title: '', description: '', image: '', category: '', price: 0, mrp: 0, isPaid: false, accessKey: '', shortenerLink: '', telegramLink: '', startDate: '', endDate: '', isNew: true });
        }
    }, [editing, showModal]);
                             ...
                             {courses.map(c => (
                                 <div key={c.id} className="bg-white p-4 rounded-[28px] flex items-center gap-4 border border-gray-100 shadow-sm">
                                     ...
                                     <div className="flex gap-2">
                                         ...
                                         <button onClick={() => { setEditing(c); setShowModal(true); }} className="p-2 bg-gray-50 text-gray-400 rounded-xl hover:bg-blue-600 hover:text-white"><Edit className="w-4 h-4" /></button>
                                         <button onClick={() => { if(confirm('Erase this batch?')) deleteCourse(c.id); }} className="p-2 bg-red-50 text-red-400 rounded-xl hover:bg-red-500 hover:text-white"><Trash2 className="w-4 h-4" /></button>
                                     </div>
                                 </div>
                             ))}
                         </div>
                    ...
                    <div className="bg-white rounded-[32px] shadow-sm border border-gray-100 overflow-hidden animate-fade-in">
                      ...
                      {users.map(u => (
                          <div key={u.id} className="p-5 flex justify-between items-center hover:bg-gray-50 transition-colors">
                              <div><p className="font-bold text-gray-800">{u.name}</p><p className="text-[11px] text-gray-400">{u.email}</p></div>
                 ...
                 <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    <div className="bg-white w-full max-w-lg rounded-[40px] p-8 shadow-2xl max-h-[90vh] overflow-y-auto no-scrollbar border border-gray-100 animate-slide-up">
                        <div className="flex justify-between items-center mb-8">
                            <h2 className="text-2xl font-bold text-gray-800 tracking-tight">{editing ? 'Edit Batch' : 'Initialize New Batch'}</h2>
                        ...
                        <form onSubmit={handleSaveCourse} className="space-y-5">
                            <div className="space-y-2">
                                    ...
                                    {form.image ? (
                                        <>
                                            ...
                                            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                                                <button type="button" onClick={() => setForm(f => ({...f, image: ''}))} className="bg-white text-red-500 p-2.5 rounded-full shadow-lg"><Trash2 className="w-5 h-5" /></button>
                                                <label className="bg-white text-blue-500 p-2.5 rounded-full shadow-lg cursor-pointer">
                                                    <Upload className="w-5 h-5" /><input type="file" className="hidden" accept="image/*" onChange={handleFileUpload} />
                                                </label>
                                            </div>
                                        </>
                                    ) : (
                                        <div className="text-center p-6 flex flex-col items-center">
                                            ...
                                            <label className="px-6 py-2.5 bg-blue-600 text-white rounded-xl text-xs font-bold cursor-pointer hover:bg-blue-700 transition-colors flex items-center gap-2">
                                ...
                                <input value={form.image} onChange={e => setForm(f => ({ ...f, image: e.target.value }))} className="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl font-mono text-[10px] shadow-sm outline-none focus:ring-2 focus:ring-blue-500/20" placeholder="Or paste image URL here..." />
                            </div>
                            <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} className="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none focus:border-blue-500 font-bold shadow-sm" placeholder="Course Title" required />
                            <div className="grid grid-cols-2 gap-4">
                                <input value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))} className="p-4 bg-gray-50 border border-gray-100 rounded-2xl text-xs uppercase font-bold shadow-sm" placeholder="Category" required />
                                <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-2xl border border-gray-100 shadow-sm cursor-pointer" onClick={() => setForm(f => ({...f, isNew: !f.isNew}))}>
                    ...
                    {filteredCourses.map(c => (
                        <div key={c.id} className="bg-white rounded-[40px] overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-500 animate-slide-up">
                            <div className="p-7 flex justify-between items-center">
                                ...
                                {c.isNew && <span className="bg-[#fff9e6] text-[#eab308] text-[9px] font-black px-2.5 py-1 rounded-lg border border-yellow-200">New</span>}
                            </div>
                            <div className="px-7 relative group">
                                <img src={c.image} className="w-full aspect-[16/9] object-cover rounded-[30px]" alt={c.title} />
                                {!c.isPaid && <div className="absolute top-4 left-11 bg-white/95 backdrop-blur-md px-3.5 py-1.5 rounded-xl text-[10px] font-black text-gray-800 shadow-xl ring-1 ring-black/5 uppercase">Free Node</div>}
                            </div>
                            <div className="p-7 pt-5 space-y-6">
                                <div className="flex gap-4">
                                    <button className="flex-1 py-4 text-sm font-black text-[#7c5cdb] border-2 border-[#7c5cdb]/10 rounded-2xl">Archive</button>
                                    <Link to={`/course/${c.id}`} className="flex-[1.5] py-4 bg-[#7c5cdb] text-white text-center text-sm font-black rounded-2xl shadow-xl shadow-[#7c5cdb]/20 active:scale-95 transition-all">Let's Study</Link>
                                </div>
                            </div>
                        </div>
                    ))}
        ...
        <div className="pb-24 pt-0 min-h-screen bg-white">
            <div className="sticky top-0 z-50 bg-white border-b border-gray-100 shadow-sm">
                <div className="flex items-center justify-between p-4 px-6">
                    ...
                    <div className="flex items-center gap-4"><XPBadge /><Bell className="w-6 h-6 text-gray-600" /></div>
                </div>
                <div className="flex px-6 gap-8 overflow-x-auto no-scrollbar">
                    {(['Description', 'Subjects', 'Resources', 'Tests', 'Community'] as const).map(tab => (
                        <button key={tab} onClick={() => setActiveTab(tab)} className={`pb-3 text-sm font-black whitespace-nowrap transition-all border-b-4 ${activeTab === tab ? 'text-brand border-brand' : 'text-gray-400 border-transparent'}`}>{tab}</button>
                    ))}
                </div>
            </div>
            <Banner />
            <div className="p-6">
                {activeTab === 'Subjects' && (
                    <div className="space-y-5">
                        {course.subjects.map((sub) => (
                            <div key={sub.id} onClick={() => navigate(`/course/${course.id}/subject/${sub.id}`)} className="bg-white border border-gray-100 rounded-[35px] p-6 flex items-center gap-6 shadow-sm active:scale-[0.98] transition-all hover:border-brand/30">
                ...
                {activeTab === 'Description' && (
                    <div className="space-y-8 animate-fade-in">
                        ...
                        <div><h2 className="text-3xl font-black text-gray-800 mb-4 tracking-tight">{course.title}</h2><p className="text-gray-500 text-base leading-relaxed font-medium">{course.description}</p></div>
                    </div>
                )}
            </div>
        </div>
...
const SubjectDetail = () => {
    ...
    const handleGenerateNotes = async () => {
        ...
        try {
            ...
            const response = await ai.models.generateContent({ model: 'gemini-3-flash-preview', contents: `Notes for ${subject.title}: ${subject.chapters.map(c => c.title).join(', ')}` });
            setNotes(response.text || "Synthesis failed.");
        } catch (e) { alert("AI failure."); } finally { setGeneratingNotes(false); }
    };
        ...
        <div className="pb-24 pt-0 min-h-screen bg-white">
            <div className="sticky top-0 z-50 bg-white border-b border-gray-100 shadow-sm">
                <div className="flex items-center justify-between p-4 px-6">
                    ...
                    <div className="flex items-center gap-4">
                        <button onClick={handleGenerateNotes} className="text-brand p-2.5 rounded-2xl hover:bg-brand/5 border border-brand/10">{generatingNotes ? <Loader2 className="w-6 h-6 animate-spin" /> : <Sparkles className="w-6 h-6" />}</button>
                        <XPBadge />
                    </div>
                </div>
            ...
            <div className="p-6 space-y-8 pb-32">
                {tab === 'Chapters' ? (
                  subject.chapters.length === 0 ? <div className="text-center py-32 opacity-30 italic font-black uppercase text-[10px]">No sequences found</div> : (
                    subject.chapters.map((chap, idx) => (
                        <div key={chap.id} onClick={() => setTab('Notes')} className="bg-white border border-gray-100 rounded-[40px] p-8 shadow-sm flex justify-between items-center active:scale-[0.98] transition-all hover:border-brand/20">
                            <div>
                                <span className="inline-block bg-brand/5 text-brand text-[10px] font-black px-3 py-1.5 rounded-xl mb-3 border border-brand/10 uppercase tracking-[0.2em]">UNIT - {String(idx+1).padStart(2, '0')}</span>
                  ...
                  <div className="space-y-6">
                    <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2">
                      {(['All', 'Lectures', 'Notes', 'DPPs'] as const).map(f => (
                        <button key={f} onClick={() => setFilter(f)} className={`px-6 py-2.5 rounded-full text-xs font-black whitespace-nowrap uppercase border-2 ${filter === f ? 'bg-[#333] border-[#333] text-white shadow-lg' : 'bg-white border-gray-100 text-gray-400'}`}>{f}</button>
                      ))}
                    </div>
                    {subject.chapters.flatMap(c => c.videos).filter(v => filter === 'All' || (filter === 'Lectures' && v.type === 'lecture')).map(v => (
                      <div key={v.id} className="bg-white border border-gray-100 rounded-[35px] p-5 shadow-sm flex gap-5 animate-slide-up group">
                        <div className="w-28 aspect-video bg-gray-50 rounded-2xl overflow-hidden shrink-0 flex items-center justify-center border border-gray-100"><PlayCircle className="w-8 h-8 text-brand/20" /></div>
                        <div className="flex-1 min-w-0">
                          <p className="text-[9px] font-black text-gray-400 uppercase tracking-[0.3em] mb-1.5">{v.type?.toUpperCase()} • {v.date}</p>
                          <h4 className="text-sm font-black text-gray-800 line-clamp-2 leading-tight tracking-tight mb-3">{v.title}</h4>
                          <button onClick={() => navigate(`/watch/${courseId}`)} className="flex items-center gap-2 px-4 py-2 bg-brand/5 text-brand rounded-xl text-[10px] font-black hover:bg-brand hover:text-white transition-all">Initialize</button>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {notes && (
                <div className="fixed inset-0 z-[100] bg-black/70 backdrop-blur-xl flex items-center justify-center p-4" onClick={() => setNotes(null)}>
                    <div className="bg-white w-full max-w-lg max-h-[85vh] rounded-[50px] p-10 overflow-y-auto shadow-2xl relative" onClick={e => e.stopPropagation()}>
                        <div className="flex justify-between items-center mb-8 sticky top-0 bg-white/80 backdrop-blur pb-4">
                            <h2 className="text-2xl font-black flex items-center gap-3"><Sparkles className="w-7 h-7 text-brand" /> Neural Summary</h2>
                            <button onClick={() => setNotes(null)} className="p-2 text-gray-400"><X /></button>
                        </div>
        ...
        <div className="pb-24 pt-24 px-6 min-h-screen bg-[#f8fafc]">
            <div className="max-w-2xl mx-auto space-y-8">
                <div className="bg-white rounded-[60px] p-12 shadow-sm border border-gray-100 relative overflow-hidden text-center">
                    <div className="w-32 h-32 rounded-[45px] bg-brand flex items-center justify-center text-white text-5xl font-black mx-auto mb-8">{currentUser.name.charAt(0)}</div>
                    <h2 className="text-4xl font-black text-gray-800 tracking-tighter uppercase mb-2">{currentUser.name}</h2>
                    <p className="text-brand font-black text-xs uppercase tracking-[0.4em] bg-brand/5 px-6 py-2 rounded-full inline-block mb-10">{currentUser.role} IDENTITY</p>
                    <div className="mt-6 space-y-4"><div className="p-6 bg-gray-50 rounded-[35px] text-gray-500 font-bold border border-gray-100/50">{currentUser.email}</div></div>
                    <button onClick={logout} className="mt-14 w-full py-6 text-red-500 font-black bg-red-50 rounded-[30px] active:scale-95 transition-all uppercase tracking-[0.4em] text-xs">DISCONNECT</button>
                </div>
    ...
    return (
        <div className="min-h-screen flex flex-col items-center justify-center p-8 bg-[#7c5cdb] relative overflow-hidden">
            <div className="text-center mb-14 relative z-10">
              <div className="w-24 h-24 bg-white rounded-[40px] flex items-center justify-center mx-auto mb-6 text-[#7c5cdb] text-4xl font-black">S</div>
              <h1 className="text-3xl font-black text-white italic tracking-tighter uppercase drop-shadow-xl">Study Tool Hub</h1>
            </div>
            <div className="bg-white p-12 rounded-[70px] w-full max-w-md shadow-2xl relative z-10 border border-white/20">
                <h2 className="text-4xl font-black text-gray-800 mb-12 text-center uppercase tracking-tighter italic">Identity</h2>
                <form onSubmit={e => { e.preventDefault(); if(isS) signup(f.name, f.email, '', f.pass); else if (!login(f.email, f.pass)) alert('Identity rejected.'); }} className="space-y-5">
                    {isS && <input className="w-full p-6 bg-gray-50 rounded-3xl outline-none font-black" placeholder="IDENTITY NAME" value={f.name} onChange={e => setF({...f, name: e.target.value})} required />}
                    <input className="w-full p-6 bg-gray-50 rounded-3xl outline-none font-black" placeholder="SIGNAL / EMAIL" value={f.email} onChange={e => setF({...f, email: e.target.value})} required />
                    <input className="w-full p-6 bg-gray-50 rounded-3xl outline-none font-black" type="password" placeholder="PASS-SEQUENCE" value={f.pass} onChange={e => setF({...f, pass: e.target.value})} required />
                    <button className="w-full py-6 bg-[#7c5cdb] text-white font-black rounded-[35px] shadow-2xl active:scale-95 transition-all mt-8">Initialize</button>
                </form>
