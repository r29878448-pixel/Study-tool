...
const Banner = () => (
  <div className="bg-blue-50 px-4 py-2 flex items-center justify-between text-[11px] font-bold text-blue-700 border-b border-blue-100">
    <span>Tip: Finish all items to reach 100% progress!</span>
...
const ShortLinkRedirect = () => {
    ...
    useEffect(() => {
        const course = courses.find(c => c.shortenerLink === slug || c.id === slug);
        if (course) navigate(`/course/${course.id}`, { replace: true });
        else navigate('/', { replace: true });
    }, [slug, courses, navigate]);
...
const TempAccessHandler = () => {
    ...
    const course = courses.find(c => c.id === id || c.shortenerLink === id);
    ...
    useEffect(() => { 
        ...
        if (currentUser && course && status === 'idle') handleVerify(); 
    }, [currentUser, course, id]);

    const handleVerify = () => {
        if (!currentUser) { navigate('/login', { state: { from: location } }); return; }
        if (!course) { setStatus('failed'); return; }
        setStatus('verifying');
        setTimeout(() => { 
        ...
        <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-gray-50">
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 max-w-sm w-full text-center">
                <div className="w-14 h-14 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-5">
                    {status === 'verifying' ? <Loader2 className="w-7 h-7 text-blue-600 animate-spin" /> : 
                     status === 'success' ? <CheckCircle className="w-7 h-7 text-green-500" /> : 
                     status === 'failed' ? <X className="w-7 h-7 text-red-500" /> : <Shield className="w-7 h-7 text-blue-600" />}
                </div>
                <h2 className="text-lg font-bold text-gray-800 mb-2">
                    {status === 'verifying' ? 'Checking Link...' : status === 'success' ? 'Access OK!' : status === 'failed' ? 'Invalid Link' : '24h Free Access'}
                </h2>
                <p className="text-gray-500 text-xs mb-8">
                    {status === 'verifying' ? 'Verifying access token...' : 
                     status === 'success' ? 'Redirecting to content...' : 
                     status === 'failed' ? 'The link you followed is invalid or expired.' : `Activating temporary access for: ${course?.title || 'this batch'}.`}
                </p>
                {status === 'idle' && course && <button onClick={handleVerify} className="w-full py-3.5 bg-blue-600 text-white font-bold rounded-xl active:scale-95 transition-all shadow-md shadow-blue-200">Claim Access</button>}
                {status === 'failed' && <button onClick={() => navigate('/')} className="w-full py-3.5 bg-gray-200 text-gray-700 font-bold rounded-xl">Go Home</button>}
            </div>
        </div>
    );
};
...
const ContentManager = ({ course, onClose }: { course: Course, onClose: () => void }) => {
    ...
    const addVideo = (sId: string, cId: string) => {
        ...
        const url = prompt('Stream Link (Youtube/Vimeo/Direct):');
...
const AdminPanel = () => {
    ...
    const copyTempLink = (c: Course) => {
        const idToUse = c.shortenerLink || c.id;
        const link = `${window.location.origin}/#/temp-access/${idToUse}`;
                             ...
                             {courses.map(c => (
                                 <div key={c.id} className="bg-white p-3.5 rounded-xl border border-gray-100 shadow-sm flex flex-col gap-3">
                                     ...
                                     <div className="flex gap-1.5 w-full">
                                        ...
                                        <button onClick={() => copyTempLink(c)} className="p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100" title="Temp Access Link"><LinkIcon className="w-4 h-4" /></button>
                                        ...
                                        <button onClick={() => { if(confirm('Permanently delete batch?')) deleteCourse(c.id); }} className="p-2 bg-red-50 text-red-400 rounded-lg hover:bg-red-500 hover:text-white transition-all"><Trash2 className="w-4 h-4" /></button>
                                     </div>
                                 </div>
                             ))}
                         </div>
                ...
                {subject.chapters.length === 0 ? (
                    ...
                    <div key={chap.id} className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm space-y-5">
                        ...
                        <div className="grid gap-2">
                            {chap.videos.map(v => (
                                <button key={v.id} onClick={() => navigate(`/watch/${courseId}?v=${v.id}`)} className="w-full p-3.5 bg-gray-50 rounded-xl flex items-center gap-3.5 hover:bg-blue-600 hover:text-white transition-all text-left group">
...
const WatchPage = () => {
    ...
    const initialVidId = query.get('v');
    ...
    useEffect(() => {
        ...
        const found = initialVidId ? allVideos.find(v => v.id === initialVidId) : allVideos[0];
                ...
                <div className="flex-1 bg-black flex items-center justify-center p-2 relative">
                    {activeVideo ? (
                        <VideoPlayer 
                            ...
                            key={activeVideo.id} // Important for re-mounting when switching videos
                        />
                    ) : (
                        <div className="text-gray-500 text-xs font-bold tracking-widest animate-pulse">SYNCHRONIZING STREAM...</div>
                    )}
                </div>
                            ...
                            <div key={sub.id} className="space-y-2">
                                ...
                                {sub.chapters.flatMap(c => c.videos).map(v => (
                                    <button 
                                        ...
                                        onClick={() => {
                                            setActiveVideo(v);
                                            navigate(`/watch/${course.id}?v=${v.id}`, { replace: true });
                                        }} 
                                        className={`w-full p-3 rounded-lg flex items-center gap-3 transition-all text-left border ${activeVideo?.id === v.id ? 'bg-blue-600/10 border-blue-500/30 shadow-inner' : 'hover:bg-white/5 border-transparent'}`}
                                    >
