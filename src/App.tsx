...
const MyCourses = () => {
  ...
  return (
    <div className="pb-24 pt-24 p-6 min-h-screen">
       ...
       {myCourses.length === 0 ? (
           <div className="text-center py-20 bg-white/80 backdrop-blur rounded-3xl border border-dashed border-gray-200">
        ...
        <div className="pb-24 pt-24 p-6 min-h-screen">
             ...
             <div className="flex gap-2 overflow-x-auto pb-4 mb-4 no-scrollbar">
                 {['courses', 'users', 'banners', 'settings'].map(t => (
                     <button key={t} onClick={() => setTab(t as any)} className={`px-5 py-2.5 rounded-xl font-bold capitalize whitespace-nowrap transition-all ${tab === t ? 'bg-brand text-white shadow-lg shadow-brand/30' : 'bg-white/80 text-gray-600 shadow-sm'}`}>{t === 'courses' ? 'Batches' : t}</button>
                 ))}
             </div>
             {tab === 'courses' && (
                 <div className="space-y-4">
                     <button onClick={() => { setEditingCourse(null); setShowCourseModal(true); }} className="w-full py-4 bg-white/70 border-2 border-dashed border-gray-300 rounded-2xl text-gray-400 font-bold flex items-center justify-center gap-2 hover:border-brand hover:text-brand transition-all"><Plus className="w-5 h-5" /> Add New Batch</button>
                     {courses.map(c => (
                         <div key={c.id} className="bg-white/80 backdrop-blur p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-4 items-start md:items-center">
